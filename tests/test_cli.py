...
def test_unknown_model_exits_nonzero_and_lists_available_ids() -> None:
    ...
    for available in ("deepseek-chat", "claude-sonnet-4-5", "kimi-k2-0905-preview"):
...
def test_parse_slash_command() -> None:
    ...
    assert parse_slash_command("/model deepseek-chat") == ("model", "deepseek-chat")
...
def test_api_key_env_name() -> None:
    assert api_key_env_name(get_model("deepseek-chat")) == "DEEPSEEK_API_KEY"
...
def test_find_first_kept_entry_id_keeps_everything_when_it_fits() -> None:
    ...
    assert find_first_kept_entry_id(session, get_model("deepseek-chat")) == first
...
def test_find_first_kept_entry_id_ignores_messages_before_last_compaction() -> None:
    ...
    assert find_first_kept_entry_id(session, get_model("deepseek-chat")) == recent
